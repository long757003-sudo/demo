...
function _collectStep0Html(data) {
  ...
  return (
    '<div class="card">' +
      '<div class="card-title">征集说明配置</div>' +
      '<div class="form-grid">' +
        '<div class="form-item">' +
      ...
      '<div class="form-footer">' +
        ...
        '<button class="btn btn-primary" onclick="' +
          ...
          'if(!t||!d){toast(\'请填写征集标题和征集说明\',\'warning\');return;}' +
  ...
  return (
    '<div class="card">' +
      ...
      '<div class="form-grid">' +
        ...
        '</div>' +
        '<div class="form-item">' +
          '<label class="form-label">联系人姓名 <span class="req">*</span></label>' +
          '<input class="form-control" id="cn-contact" value="' + (data.contactName || '张华') + '">' +
        '</div>' +
        '<div class="form-item span-2">' +
          '<label class="form-label">通知正文 <span class="req">*</span></label>' +
          '<textarea class="form-control" id="cn-body" rows="4" placeholder="请输入通知正文...">' + (data.notifBody || '') + '</textarea>' +
        '</div>' +
        '<div class="form-item">' +
          '<label class="form-label">联系电话 <span class="req">*</span></label>' +
          '<input class="form-control" id="cn-phone" placeholder="请输入联系电话" value="' + (data.contactPhone || '023-68253188') + '">' +
        '</div>' +
        '<div class="form-item">' +
          '<label class="form-label">联系邮箱 <span class="req">*</span></label>' +
          '<input class="form-control" id="cn-email" placeholder="请输入联系邮箱" value="' + (data.contactEmail || 'zhanghua@swu.edu.cn') + '">' +
        '</div>' +
        '<div class="form-item span-2">' +
          ...
          '<div style="display:flex;flex-wrap:wrap;gap:16px;padding-top:6px">' +
            ...
            '<label><input type="checkbox" id="cn-r-role" onchange="_updateRecipientCount()"> 按角色类型选择（如勾选某角色则自动包含所有单位该角色人员）</label>' +
      ...
      '<div class="form-footer">' +
        ...
        '<button class="btn btn-primary" onclick="' +
          ...
          'window._collectWizard.data.contactName=document.getElementById(\'cn-contact\').value;' +
          'window._collectWizard.data.contactPhone=document.getElementById(\'cn-phone\').value;' +
          'window._collectWizard.data.contactEmail=document.getElementById(\'cn-email\').value;' +
  ...
  const nodeHtml = defaultNodes.map(function(node, i) {
    ...
    '<div style="display:inline-flex;flex-direction:column;align-items:center;gap:4px">' +
      ...
      '<div style="display:none;background:#f5f5f5;border:1px solid #e0e0e0;border-radius:4px;padding:6px;min-width:90px;font-size:11px">' +
        materialCheckboxes +
      '</div>' +
      '<button class="btn btn-sm" style="font-size:10px;padding:1px 6px;margin-top:2px" ' +
        'onclick="toast(\'Demo：评审节点已插入\',\'success\')">+ 插入评审节点</button>' +
    '</div>';
  }).join('');
    ...
    '<div class="card">' +
      ...
      '<div class="card" style="border:1px solid #e0e0e0;margin-bottom:16px">' +
        ...
        '<div class="detail-grid">' +
          ...
          '<div class="detail-item"><span class="detail-label">联系电话</span><span class="detail-value">' + (data.contactPhone || '023-68253188') + '</span></div>' +
          '<div class="detail-item"><span class="detail-label">联系邮箱</span><span class="detail-value">' + (data.contactEmail || 'zhanghua@swu.edu.cn') + '</span></div>' +
      ...
      '<div class="card" style="border:1px solid #e0e0e0;margin-bottom:16px">' +
        ...
        '</div>' +
        '<div style="margin-bottom:8px;font-size:13px"><b>模板：</b>' + tplName + ' &nbsp; <b>材料节点数：</b>6 个节点 / 共 12 项材料</div>' +
...
registerView('collection-create', function() {
  ...
  var contactDefault = (typeof roleDisplayName === 'function' ? roleDisplayName(getCurrentRole()) : '') || '林已杰 20054379';

  var unitList = ['教务处','招生处','科研处','学工处','党政办','图书馆','信息化办','资产处'];
  var unitCheckboxes = unitList.map(function(u) {
    return '<label style="display:inline-flex;align-items:center;gap:5px;min-width:90px;cursor:pointer">' +
      '<input type="checkbox" name="cc-unit-cb" value="' + u + '"' + (draft.units && draft.units.indexOf(u) >= 0 ? ' checked' : '') + '> ' + u +
    '</label>';
  }).join('');

  var sendModeOpts = [
    { value: 'system',      label: '系统消息' },
    { value: 'system+dd',   label: '系统+钉钉' },
    { value: 'system+dd+sms', label: '系统+钉钉+短信' },
  ];
  var sendModeRadios = sendModeOpts.map(function(o) {
    var checked = (draft.sendMode === o.value) || (!draft.sendMode && o.value === 'system+dd') ? ' checked' : '';
    return '<label style="display:inline-flex;align-items:center;gap:5px;cursor:pointer;margin-right:16px">' +
      '<input type="radio" name="cc-send-mode" value="' + o.value + '"' + checked + '> ' + o.label +
    '</label>';
  }).join('');

  return (
    breadcrumb('需求征集', '发起征集') +
    '<h2 class="page-title">发起需求征集</h2>' +

    '<div class="card">' +
      '<div class="card-title">征集基本信息</div>' +
      '<div class="form-grid">' +

        /* 征集批次标题 */
        '<div class="form-item" style="grid-column:1/-1">' +
          '<label class="form-label required">征集批次标题' +
            '<span class="tooltip-icon" data-tip="建议格式「年度+项目类型+需求征集」">?</span>' +
          '</label>' +
          '<input class="form-control" id="cc-title" type="text" maxlength="60" ' +
            'placeholder="2026年信息化项目需求征集" value="' + (draft.title || '') + '">' +
        '</div>' +

        /* 征集说明 */
        '<div class="form-item" style="grid-column:1/-1">' +
          '<label class="form-label">征集说明</label>' +
          '<textarea class="form-control" id="cc-desc" rows="3" placeholder="请输入征集说明（选填）">' + (draft.desc || '') + '</textarea>' +
        '</div>' +

        /* 开始日期 */
        '<div class="form-item">' +
          '<label class="form-label required">开始日期</label>' +
          '<input class="form-control" id="cc-start" type="date" value="' + (draft.startDate || '') + '">' +
        '</div>' +

        /* 截止日期 */
        '<div class="form-item">' +
          '<label class="form-label required">截止日期' +
            '<span class="tooltip-icon" data-tip="Demo 不做时间锁定，任何时候均可操作">?</span>' +
          '</label>' +
          '<input class="form-control" id="cc-end" type="date" value="' + (draft.endDate || '') + '">' +
        '</div>' +

        /* 附件 */
        '<div class="form-item" style="grid-column:1/-1">' +
          '<label class="form-label">附件上传（选填）</label>' +
          '<input class="form-control" id="cc-file" type="file" accept=".pdf,.doc,.docx">' +
          '<div style="font-size:11px;color:var(--text-secondary);margin-top:4px">仅演示，不真实上传</div>' +
        '</div>' +

        /* 通知接收单位 */
        '<div class="form-item" style="grid-column:1/-1">' +
          '<label class="form-label required">通知接收单位' +
            '<span class="tooltip-icon" data-tip="将自动向所选单位的系统管理员发送征集通知">?</span>' +
          '</label>' +
          '<div id="cc-units" style="display:flex;flex-wrap:wrap;gap:8px 12px;padding:10px;border:1px solid #d9d9d9;border-radius:6px;background:#fafafa">' +
            unitCheckboxes +
          '</div>' +
        '</div>' +
        ...
        /* 通知联系人 */
        '<div class="form-item">' +
          '<label class="form-label">通知联系人</label>' +
          '<input class="form-control" id="cc-contact" type="text" value="' + (draft.contact || contactDefault) + '">' +
        '</div>' +

        /* 发送方式 */
        '<div class="form-item">' +
          '<label class="form-label required">发送方式</label>' +
          '<div id="cc-send-mode" style="display:flex;flex-wrap:wrap;padding-top:6px">' +
            sendModeRadios +
          '</div>' +
        '</div>' +
    ...
    /* 流程配置 */
    '<fieldset class="card" style="border:1px solid #d9d9d9;margin-top:0">' +
      '<legend style="padding:0 8px;font-weight:600">流程配置</legend>' +
      '<div style="display:flex;flex-direction:column;gap:10px;padding:8px 0">' +
        '<label style="display:inline-flex;align-items:center;gap:8px;cursor:pointer">' +
          '<input type="checkbox" id="cc-flow-sort"' + (draft.flow && draft.flow.sort === false ? '' : ' checked') + '> 单位排序环节' +
        '</label>' +
        '<label style="display:inline-flex;align-items:center;gap:8px;cursor:pointer">' +
          '<input type="checkbox" id="cc-flow-approve"' + (draft.flow && draft.flow.approve === false ? '' : ' checked') + '> 单位领导审批环节' +
        '</label>' +
        '<label style="display:inline-flex;align-items:center;gap:8px;cursor:pointer">' +
          '<input type="checkbox" id="cc-flow-expert"' + (draft.flow && draft.flow.expert ? ' checked' : '') + '>' +
          ' 专家评审环节（Phase 3 后启用）' +
          '<span class="tooltip-icon" data-tip="Phase 3 实现，当前仅展示开关占位">?</span>' +
        '</label>' +
        '<label style="display:inline-flex;align-items:center;gap:8px;cursor:pointer">' +
          '<input type="checkbox" id="cc-flow-notify"' + (draft.flow && draft.flow.notify === false ? '' : ' checked') + '> 遴选结果通知' +
        '</label>' +
      '</div>' +
      '<div style="margin-top:8px">' +
        '<label style="font-size:13px;color:var(--text-secondary)">流程模板：</label>' +
        '<select class="form-control" style="width:220px;display:inline-block" disabled>' +
          '<option>标准需求征集流程</option>' +
        '</select>' +
      '</div>' +
    '</fieldset>' +
